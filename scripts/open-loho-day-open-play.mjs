/**
 * 為樂活板橋（loho2）指定日期開放每小時臨打（球敘 OPEN_PLAY，三場 A/B/C，09:00–24:00）。
 *
 *   npx tsx scripts/open-loho-day-open-play.mjs
 *   npx tsx scripts/open-loho-day-open-play.mjs 2026-06-05
 *   npx tsx scripts/open-loho-day-open-play.mjs --dry-run
 */
...
async function main() {
  ...
  console.log(`時段：每日 ${HOUR_START}:00–${HOUR_END}:00（每小時臨打，容量 ${DEFAULT_CAPACITY} 人）`);
  console.log(dryRun ? "模式：預覽\n" : "模式：正式建立\n");
  console.log(`將建立 ${toCreate.length} 個臨打時段`);

  if (!dryRun && toCreate.length > 0) {
    for (const data of toCreate) {
      await prisma.activity.create({ data });
    }
    console.log("完成。若同時有租場，看板格會顯示「臨打＋租場」。");
  }
