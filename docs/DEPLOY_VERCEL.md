...
| `SMTP_*` | Email 通知 |
| `DUPR_*` | DUPR API |
| `SKILL_ANALYZER_URL` | Railway worker 公開網址（見 `services/skill-analyzer/README.md`）。**Vercel 無法跑 YOLO**，未設則前台顯示「分析服務未連線」 |
| `SKILL_ANALYZER_SECRET` | 與 Railway worker 相同的 HMAC 密鑰 |

最少維護做法：網站在 Vercel，分析服務用 **Railway** 託管 Docker（不必自己管 VPS）。
