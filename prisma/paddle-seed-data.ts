/** Seed 用靜態球拍清單（slug = 前台 URL） */
export const SEED_PADDLES = [
  {
    slug: "luzz-inferno-zero",
    brand: "LUZZ",
    series: "地獄火",
    variant: "零度",
    nameZh: "地獄火 零度",
    nameEn: "Inferno - Zero / Frozen",
    description:
      "地獄火系列的零度版本，主打清爽配色與穩定手感，適合喜歡均衡操控與控球的球員。",
    highlights: ["地獄火系列", "均衡手感", "控球取向"],
  },
  {
    slug: "luzz-inferno-darkness",
    brand: "LUZZ",
    series: "地獄火",
    variant: "暗黑",
    nameZh: "地獄火 暗黑",
    nameEn: "Inferno - Darkness",
    description:
      "地獄火暗黑配色，視覺壓迫感強，延續系列攻擊與節奏推進的打法特性。",
    highlights: ["地獄火系列", "暗黑配色", "攻擊節奏"],
  },
  {
    slug: "luzz-inferno-pink-purple",
    brand: "LUZZ",
    series: "地獄火",
    variant: "粉紫",
    nameZh: "地獄火 粉紫",
    nameEn: "Inferno - Pink Purple",
    description:
      "粉紫配色的地獄火，外型辨識度高，適合想要性能與風格並重的球員。",
    highlights: ["地獄火系列", "高辨識外型"],
  },
  {
    slug: "luzz-inferno-blue-flame",
    brand: "LUZZ",
    series: "地獄火",
    variant: "藍焰",
    nameZh: "地獄火 藍焰",
    nameEn: "Inferno - Blue Flame",
    description: "藍焰版本延續地獄火系列火感主題，偏攻擊推進與中前場節奏。",
    highlights: ["地獄火系列", "藍焰配色"],
  },
  {
    slug: "luzz-tornado-black",
    brand: "LUZZ",
    series: "龍捲風",
    variant: "黑",
    nameZh: "龍捲風 黑",
    nameEn: "Tornado - Black",
    description:
      "龍捲風系列黑色款，強調旋轉與變化球手感，適合喜歡製造球路變化的打法。",
    highlights: ["龍捲風系列", "旋轉變化"],
  },
  {
    slug: "luzz-tornado-purple",
    brand: "LUZZ",
    series: "龍捲風",
    variant: "紫",
    nameZh: "龍捲風 紫",
    nameEn: "Tornado - Purple",
    description:
      "紫色龍捲風，系列同調手感，外型更鮮明，適合雙打中需要靈活變速的球員。",
    highlights: ["龍捲風系列", "靈活變速"],
  },
  {
    slug: "luzz-cannon-g1-black",
    brand: "LUZZ",
    series: "一代加農砲",
    variant: "黑",
    nameZh: "一代加農砲 黑",
    nameEn: "Cannon Gen 1 - Black",
    description:
      "一代加農砲經典黑色款，以力量輸出與甜蜜點穩定度見長，適合喜歡主動進攻的球友。",
    highlights: ["一代加農砲", "力量輸出"],
  },
  {
    slug: "luzz-cannon-g1-collab",
    brand: "LUZZ",
    series: "一代加農砲",
    variant: "聯名",
    nameZh: "一代加農砲 聯名",
    nameEn: "Cannon Gen 1 - Co-branded",
    description: "一代加農砲聯名版本，保留系列攻擊性格，外觀為聯名限定設計。",
    highlights: ["一代加農砲", "聯名限定"],
  },
  {
    slug: "luzz-cannon-g1-candy",
    brand: "LUZZ",
    series: "一代加農砲",
    variant: "蜜糖",
    nameZh: "一代加農砲 蜜糖",
    nameEn: "Cannon Gen 1 - Candy / Honey",
    description: "蜜糖配色的一代加農砲，性能取向與系列一致，外型更柔和吸睛。",
    highlights: ["一代加農砲", "蜜糖配色"],
  },
  {
    slug: "luzz-cannon-g1-ex",
    brand: "LUZZ",
    series: "一代加農砲",
    variant: "EX",
    nameZh: "一代加農砲 EX",
    nameEn: "Cannon Gen 1 - EX",
    description:
      "一代加農砲 EX 版本，定位為系列進階款，適合想再多一點擊球回饋的球員。",
    highlights: ["一代加農砲", "EX 進階"],
  },
  {
    slug: "luzz-glider-2026",
    brand: "LUZZ",
    series: "滑翔機",
    variant: "2026",
    nameZh: "滑翔機 2026",
    nameEn: "Glider - 2026",
    description:
      "滑翔機 2026，強調輕盈操控與連續對拉穩定度，適合喜歡細膩控球的打法。",
    highlights: ["滑翔機系列", "輕盈操控"],
  },
  {
    slug: "luzz-glider-signature",
    brand: "LUZZ",
    series: "滑翔機",
    variant: "簽名版",
    nameZh: "滑翔機 簽名版",
    nameEn: "Glider - Signature Edition",
    description: "滑翔機簽名版，系列手感加上簽名限定外觀，收藏與實戰兼具。",
    highlights: ["滑翔機系列", "簽名限定"],
  },
  {
    slug: "luzz-glider-gatsby",
    brand: "LUZZ",
    series: "滑翔機",
    variant: "蓋比",
    nameZh: "滑翔機 蓋比",
    nameEn: "Glider - Gatsby",
    description: "滑翔機蓋比版本，延續系列控球特性，外型為特別配色款。",
    highlights: ["滑翔機系列", "特別配色"],
  },
  {
    slug: "luzz-glider-sugar",
    brand: "LUZZ",
    series: "滑翔機",
    variant: "蜜袋鼯",
    nameZh: "滑翔機 蜜袋鼯",
    nameEn: "Glider Sugar",
    description:
      "滑翔機 2026 新配色蜜袋鼯（Sugar），同平台全面／hybrid 手感，蜜袋鼯圖騰辨識度高。",
    highlights: ["滑翔機系列", "蜜袋鼯配色", "Glider 2026"],
  },
  {
    slug: "luzz-glider-falcon",
    brand: "LUZZ",
    series: "滑翔機",
    variant: "燕隼",
    nameZh: "滑翔機 燕隼",
    nameEn: "Glider Falcon",
    description:
      "滑翔機 2026 新配色燕隼（Falcon），同平台全面／hybrid 手感，燕隼圖騰與紅白雲紋。",
    highlights: ["滑翔機系列", "燕隼配色", "Glider 2026"],
  },
  {
    slug: "luzz-bladz-longyuan",
    brand: "LUZZ",
    series: "刀鋒",
    variant: "龍淵",
    nameZh: "刀鋒 龍淵",
    nameEn: "Pro Bladz 2 - Long Yuan",
    description:
      "刀鋒龍淵（Pro Bladz 2），偏精準切球與快速揮拍反應，適合前場攔截與變向。",
    highlights: ["刀鋒系列", "精準切球"],
  },
  {
    slug: "luzz-cannon-g2-black",
    brand: "LUZZ",
    series: "二代加農砲",
    variant: "黑",
    nameZh: "二代加農砲 黑",
    nameEn: "Cannon Gen 2 - Black",
    description:
      "二代加農砲黑色款，承襲加農砲攻擊基因並升級手感回饋，適合主動進攻型球員。",
    highlights: ["二代加農砲", "攻擊升級"],
  },
  {
    slug: "luzz-cannon-g2-collab",
    brand: "LUZZ",
    series: "二代加農砲",
    variant: "聯名",
    nameZh: "二代加農砲 聯名",
    nameEn: "Cannon Gen 2 - Co-branded",
    description: "二代加農砲聯名版，性能延續 Gen 2，外觀為聯名特別設計。",
    highlights: ["二代加農砲", "聯名限定"],
  },
  {
    slug: "luzz-cannon-g1-jurassic",
    brand: "LUZZ",
    series: "一代加農砲",
    variant: "侏羅紀公園聯名",
    nameZh: "一代加農砲 侏羅紀公園聯名",
    nameEn: "Cannon Gen 1 - Jurassic Edition",
    description: "一代加農砲侏羅紀公園聯名版，延續 Cannon 力量平台，外觀為 Jurassic Edition。",
    highlights: ["一代加農砲", "侏羅紀聯名"],
  },
  {
    slug: "luzz-cannon-g1-minions",
    brand: "LUZZ",
    series: "一代加農砲",
    variant: "小小兵聯名",
    nameZh: "一代加農砲 小小兵聯名",
    nameEn: "Cannon Gen 1 - Minions",
    description: "一代加農砲小小兵聯名版（有別於二代小小兵聯名），延續 Cannon 攻擊手感與聯名外觀。",
    highlights: ["一代加農砲", "小小兵聯名"],
  },
  {
    slug: "joola-pro-v-perseus",
    brand: "JOOLA",
    series: "Pro V",
    variant: "Perseus 14/16mm",
    nameZh: "Pro V Perseus",
    nameEn: "Pro V Perseus",
    description: "JOOLA Pro V Perseus（14/16mm）。Pro V 系列長版攻擊取向，厚度可選 14mm／16mm。",
    highlights: ["Pro V", "Perseus", "14/16mm"],
  },
  {
    slug: "joola-pro-v-kosmos",
    brand: "JOOLA",
    series: "Pro V",
    variant: "Kosmos 16mm",
    nameZh: "Pro V Kosmos",
    nameEn: "Pro V Kosmos",
    description: "JOOLA Pro V Kosmos（16mm）。Pro V 系列 Kosmos 款式。",
    highlights: ["Pro V", "Kosmos", "16mm"],
  },
  {
    slug: "joola-pro-v-scorpeus",
    brand: "JOOLA",
    series: "Pro V",
    variant: "Scorpeus 14/16mm",
    nameZh: "Pro V Scorpeus",
    nameEn: "Pro V Scorpeus",
    description: "JOOLA Pro V Scorpeus（14/16mm）。Pro V 系列 Scorpeus 款式。",
    highlights: ["Pro V", "Scorpeus", "14/16mm"],
  },
  {
    slug: "enhance-epp-widebody",
    brand: "ENHANCE",
    series: "EPP 寬版",
    variant: "-",
    nameZh: "EPP 寬版",
    nameEn: "EPP Widebody",
    description: "ENHANCE EPP 寬版。有別於 MPP Turbo 系列的另一產品線寬版款。",
    highlights: ["EPP", "寬版"],
  },
  {
    slug: "leopard-wave-x",
    brand: "LEOPARD",
    series: "Wave X",
    variant: "-",
    nameZh: "Wave X",
    nameEn: "Wave X",
    description: "Leopard Wave X。後續可再補規格、評測與產品圖。",
    highlights: ["Wave X"],
  },
  {
    slug: "facolos-elite-x",
    brand: "FACOLOS",
    series: "ELITE-X",
    variant: "-",
    nameZh: "ELITE-X",
    nameEn: "ELITE-X",
    description: "Facolos ELITE-X。後續可再補規格、評測與產品圖。",
    highlights: ["ELITE-X"],
  },
  {
    slug: "sypik-triton5",
    brand: "SYPIK",
    series: "TRITON5",
    variant: "-",
    nameZh: "TRITON5",
    nameEn: "TRITON5",
    description:
      "SYPIK TRITON5，強調穩定甜蜜點與全面型打法，適合想要一拍打遍攻守的球友。",
    highlights: ["全面型", "穩定甜蜜點"],
  },
  {
    slug: "zocker-aspire-signature",
    brand: "ZOCKER",
    series: "ASPIRE SIGNATURE",
    variant: "-",
    nameZh: "ASPIRE SIGNATURE",
    nameEn: "ASPIRE SIGNATURE",
    description:
      "ZOCKER Aspire Signature，簽名系列定位，兼顧外觀識別與實戰手感。",
    highlights: ["簽名系列"],
  },
  {
    slug: "enhance-mpp",
    brand: "ENHANCE",
    series: "MPP",
    variant: "-",
    nameZh: "MPP",
    nameEn: "MPP",
    description:
      "ENHANCE MPP，偏向現代全面型配置，適合想要乾淨擊球回饋的球員。",
    highlights: ["全面型"],
  },
  {
    slug: "pakle-fuse",
    brand: "PAKLE",
    series: "FUSE",
    variant: "-",
    nameZh: "FUSE",
    nameEn: "FUSE",
    description:
      "PAKLE FUSE，融合操控與力量的中庸取向，適合進階球友作為主力拍。",
    highlights: ["操控與力量平衡"],
  },
  {
    slug: "rpm-q2",
    brand: "RPM",
    series: "Q2",
    variant: "-",
    nameZh: "Q2",
    nameEn: "Q2",
    description:
      "RPM Q2，強調擊球速度與中場轉換節奏，適合喜歡主動推進的球員。",
    highlights: ["速度節奏"],
  },
  {
    slug: "rpm-v2",
    brand: "RPM",
    series: "V2",
    variant: "-",
    nameZh: "V2",
    nameEn: "V2",
    description:
      "RPM V2，系列另一主力款，手感與平衡取向不同於 Q2，適合依打法分開挑選。",
    highlights: ["系列主力"],
  },
  {
    slug: "honolulu-j2cr",
    brand: "HONOLULU",
    series: "J2CR",
    variant: "-",
    nameZh: "J2CR",
    nameEn: "J2CR",
    description:
      "HONOLULU J2CR，以控制與擊球精度著稱，適合喜歡細膩擺放與軟手過渡的打法。",
    highlights: ["控制精度", "軟手過渡"],
  },
  {
    slug: "honolulu-j6cr",
    brand: "HONOLULU",
    series: "J6CR",
    variant: "-",
    nameZh: "J6CR",
    nameEn: "J6CR",
    description:
      "HONOLULU J6CR，與 J2CR 為不同款式，同樣強調控制手感，適合依手感喜好分開挑選。",
    highlights: ["控制精度"],
  },
  {
    slug: "luzz-inferno-dawn",
    brand: "LUZZ",
    series: "地獄火",
    variant: "破曉",
    nameZh: "地獄火 破曉",
    nameEn: "Inferno2 M1 Dawn",
    description:
      "地獄火二代（Inferno2 M1／破曉），延續一代力量取向，升級核心與耐用旋貼。",
    highlights: ["地獄火", "破曉", "Inferno2 M1"],
  },
  {
    slug: "spartus-spitfire",
    brand: "SPARTUS",
    series: "Spitfire",
    variant: "-",
    nameZh: "Spitfire",
    nameEn: "Spitfire Elongated",
    description: "Spartus Spitfire 長版，高性價比力量拍，旋轉與耐用旋貼是賣點。",
    highlights: ["Spitfire", "長版", "Gen4"],
  },
  {
    slug: "rpm-jade",
    brand: "RPM",
    series: "Jade",
    variant: "14/16mm",
    nameZh: "Jade",
    nameEn: "Jade",
    description: "RPM Jade 全泡沫力量拍，主打耐用旋貼與長／寬版、14／16mm 選擇。",
    highlights: ["Jade", "MPP", "SICmatrix"],
  },
  {
    slug: "six-zero-coral-pro",
    brand: "SIX ZERO",
    series: "Coral Pro",
    variant: "16mm",
    nameZh: "Coral Pro",
    nameEn: "Coral Pro 16mm",
    description: "Six Zero Coral Pro 16mm，Coral 線進階款，旋與柔軟力量並重。",
    highlights: ["Coral Pro", "16mm"],
  },
  {
    slug: "selkirk-omni",
    brand: "SELKIRK",
    series: "OMNI",
    variant: "-",
    nameZh: "OMNI",
    nameEn: "OMNI",
    description: "Selkirk OMNI 全面型泡沫拍，可調 MOI，偏控制與全場適應。",
    highlights: ["OMNI", "全面型", "MOI"],
  },
  {
    slug: "selkirk-boomstik",
    brand: "SELKIRK",
    series: "LABS Boomstik",
    variant: "-",
    nameZh: "Boomstik",
    nameEn: "LABS Project Boomstik",
    description: "Selkirk LABS Boomstik 力量泡沫旗艦，主打爆炸 pop 與進攻壓力。",
    highlights: ["Boomstik", "LABS", "力量"],
  },
  {
    slug: "selkirk-boomstik-asia-gift",
    brand: "SELKIRK",
    series: "LABS Boomstik",
    variant: "亞洲版禮盒",
    nameZh: "Boomstik 亞洲版禮盒",
    nameEn: "LABS Project Boomstik Asia Gift Set",
    description: "Boomstik 亞洲版禮盒，性能大致對齊 LABS Boomstik 平台。",
    highlights: ["Boomstik", "亞洲版", "禮盒"],
  },
] as const;
